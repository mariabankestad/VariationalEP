...
def _searchsorted(sorted_sequence, values):
    ...
    """
    temp = torch.sum(values[..., None] >= sorted_sequence, dim=-1) - 1
    return torch.sum(values[..., None] >= sorted_sequence, dim=-1) - 1

def searchsorted(bin_locations, inputs, eps=1e-4):
    bin_locations[..., -1] += eps
    return torch.sum(
...
def rational_quadratic_spline(inputs,
    ...
    if inverse:
        ...
        discriminant = b.pow(2) - 4 * a * c + 1e-5
...
def unconstrained_rational_quadratic_spline(inputs,
                                            ...
                                            min_bin_height=1e-3,
                                            min_derivative=1e-3):
    inside_interval_mask = (inputs >= -bound) & (inputs <= bound)
    ...
    outputs[inside_interval_mask], logabsdet[inside_interval_mask] = rational_quadratic_spline(
        ...
        inverse=inverse,
        left=-bound, right=bound, bottom=-bound, top=bound,
...
def _monotonic_rational_spline(
    ...
    if inverse:
        ...
        discriminant = b.pow(2) - 4 * a * c + 1e-7
        #discriminant = clamp_preserve_gradients(discriminant,  min=0.0, max = 1e4)
        discriminant[outside_interval_mask] = 0
        outputs[outside_interval_mask] = inputs[outside_interval_mask] 

        assert(discriminant >= 0).all(), str(discriminant)
...
def conditional_spline(
    ...
    return ConditionalSpline([nn1, nn2, nn3], input_dim, count_bins, bound=bound, order=order)